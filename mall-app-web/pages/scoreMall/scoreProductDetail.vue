    ...
    <view class="container">
        ...
        <view class="introduce-section">
            ...
            <view class="price-box">
                <text class="price-tip">兑换所需积分</text>
                <text class="price">{{ product.price * 100 }} 积分</text>
            </view>
            <view class="bot-row">
        ...
        <view class="page-bottom">
            ...
            <view class="action-btn-group">
                <button type="primary" class=" action-btn no-border buy-now-btn" @click="exchange">积分兑换</button>
            </view>
        </view>
...
export default {
    ...
    methods: {
        ...
        async exchange() {
            let userIntegration = this.$store.state.userInfo.integration || 0;
            let needIntegration = this.product.price * 100;
            if (userIntegration >= needIntegration) {
                let response = await fetchCartList();
                let cartList = response.data || [];
                let matchedItems = cartList.filter(item => item.productId === this.product.id);
                let cartIds = matchedItems.map(item => item.id);
                if (cartIds.length == 0) {
                    await this.addToCart();
                    // 再次查询购物车
                    response = await fetchCartList();
                    cartList = response.data || [];
                    matchedItems = cartList.filter(item => item.productId === this.product.id);
                    cartIds = matchedItems.map(item => item.id);
                }
                // 延迟跳转，确保 cartIds 已更新
                setTimeout(() => {
                    if (cartIds.length > 0) {
                        uni.navigateTo({
                            url: `/pages/order/createOrder?source=scoreMall&cartIds=${JSON.stringify(cartIds)}`
                        });
                    }
                }, 300);
            } else {
