			...
			confirm: async function() {
				if(USE_ALIPAY){
					...
					payOrderSuccess({
						...
						if (this.userInfo && typeof this.userInfo.integration === 'number') {
							const useIntegration =  this.orderInfo.integrationAmount * 100;
							this.$store.commit('UPDATE_USER_INFO', {
							    ...this.userInfo,
							    integration: this.userInfo.integration - useIntegration + 10,
							});
						} else {
							console.warn('用户信息或积分数据无效');
						}
						uni.redirectTo({
							url: '/pages/money/paySuccess'
						});
					}).catch(error => {
						console.error('支付成功回调失败:', error);
					});
				}
