	...
	<view>
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell b-b">
				...
				<input class="integration" type="number" v-model="useIntegration"
				  :readonly="integrationInputDisabled" :disabled="integrationInputDisabled"
				  placeholder="使用积分数量" placeholder-class="placeholder"
	...
	export default {
		data() {
			return {
				...
				memberIntegration: 0,
				cartIds: [],
				integrationInputDisabled: false, // 控制积分输入框是否禁用
			}
		},
		onLoad(option) {
			//商品数据
			// 判断来源
			if (option.source === 'cart') {
				// 购物车跳转
				this.cartIds = JSON.parse(option.cartIds);
				this.loadData();
			} else if (option.source == 'scoreMall') {
				console.log('积分兑换跳转');
				this.cartIds = JSON.parse(option.cartIds);
				this.loadData();
				// 积分兑换跳转，自动填入积分并禁用输入框
				// 需在loadData后设置，需Integration=商品总价*100
				this.integrationInputDisabled = true;
				// 延迟设置积分，确保calcAmount已更新
				setTimeout(() => {
					if (this.calcAmount && this.calcAmount.totalAmount) {
						this.useIntegration = Math.round(this.calcAmount.totalAmount * 100);
					}
				}, 500);
			} else if (option.source === 'directBuy') {
				// 直接购买（非购物车/积分兑换）
				if (option.productInfo) {
					this.cartPromotionItemList = [JSON.parse(option.productInfo)];
				}
				// 其他参数同上
				if (option.memberIntegration) {
					this.memberIntegration = Number(option.memberIntegration);
				}
				if (option.calcAmount) {
					this.calcAmount = JSON.parse(option.calcAmount);
				}
				if (option.integrationConsumeSetting) {
					this.integrationConsumeSetting = JSON.parse(option.integrationConsumeSetting);
				}
				if (option.memberReceiveAddressList) {
					this.memberReceiveAddressList = JSON.parse(option.memberReceiveAddressList);
