...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    @Autowired
    private UmsIntegrationChangeHistoryServiceImpl integrationChangeHistoryMapper;
    ...
    public Map<String, Object> generateOrder(OrderParam orderParam) {
        ...
        //判断是否使用积分
        boolean isUseIntegration = false;
        if (orderParam.getUseIntegration() == null||orderParam.getUseIntegration().equals(0)) {
            ...
            Long id =  orderItemList.get(0).getProductId();
            if (orderItemList.size() == 1 && orderItemList.get(0).getProductQuantity() == 1 && homeDao.getIntegrationProductById(id)!= null){
                //判断用户是否有这么多积分
                if (orderParam.getUseIntegration().compareTo(currentMember.getIntegration()) > 0) {
                    Asserts.fail("积分不足，无法使用！");
                }else{
                    isUseIntegration = true;
                    orderItemList.get(0).setIntegrationAmount(BigDecimal.valueOf(orderParam.getUseIntegration()/100.0));
                }
            } else if (integrationAmount.compareTo(new BigDecimal(0)) == 0) {
                Asserts.fail("积分不可用");
            } else {
                //可用情况下分摊到可用商品中
                for (OmsOrderItem orderItem : orderItemList) {
        ...
        //计算order_item的实付金额
        if(isUseIntegration){
            orderItemList.get(0).setRealAmount(BigDecimal.valueOf(0));//如果只购买了一个商品，且是积分商品，将积分抵扣金额设为0
        }else{
        ...
        if (orderParam.getUseIntegration() != null) {
            ...
            integrationChangeHistoryMapper.addIntegrationChangeHistory(currentMember.getId(), 1, -1*orderParam.getUseIntegration(), 0, "订单支付使用积分");
        }
     ...
     * 增加会员积分
     */
    public void addIntegration(Long orderId, Integer integration) {
        if (integration == null) {
            return;
        }
        OmsOrder order = orderMapper.selectByPrimaryKey(orderId);
        ...
        integrationChangeHistoryService.addIntegrationChangeHistory(member.getId(), 0, integration, 4,"下单赠送积分");
    }
}
