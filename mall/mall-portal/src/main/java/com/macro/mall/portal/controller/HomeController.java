...
public class HomeController {
    ...
    @ApiOperation("分页获取积分商品")
    @RequestMapping(value = "/integrationProductList", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult<List<PmsProduct>> integrationProductList(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                                         @RequestParam(value = "pageSize", defaultValue = "6") Integer pageSize) {
        List<PmsProduct> productList = homeService.integrationProductList(pageNum,pageSize);
        return CommonResult.success(productList);
    }
}
